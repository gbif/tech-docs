...
The development site is visible at https://techdocs.gbif-test.org/
...
The OpenAPI source link:./fetch-openapi.py[is retrieved] whenever webservices are deployed, so it is always up-to-date.  This is done on all environments; techdocs.gbif.org documents api.gbif.org and techdocs.gbif-test.org documents api.gbif-test.org.
...
for lang in en es; do
    docker run --rm -t --user $(id -u) --volume $PWD:/antora:Z antora/antora:3.1.10 techdocs-$lang-playbook.yml
done
----
