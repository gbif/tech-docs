...
During development, it is visible at https://techdocs.gbif-uat.org/

NOTE: This file is documenting the documentation system, it's not shown on https://techdocs.gbif-uat.org/.
...
Components:: The largest blocks of documentation.  Components we have include https://techdocs.gbif-uat.org/en/data-publishing/[_Data Publishing_] and the https://techdocs.gbif-uat.org/en/openapi/[_API reference_].
...
A version of a component is defined by its `antora.yml` descriptor file, which should be present at the location specified in `{url}/{start_path}` from the playbook's `content.sources` section.  For example, the https://techdocs.gbif-uat.org/en/data-publishing/[_Data Publishing_] component has its descriptor file in this repository at link:./en/data-publishing/antora.yml[en/data-publishing/antora.yml]:
...
The English home page, https://techdocs.gbif-uat.org/en/, is a component called `ROOT`, which is a special name where `ROOT` doesn't show in links or URLs.  Reserve pages in this section for the home page and very general pages about the whole manual.
