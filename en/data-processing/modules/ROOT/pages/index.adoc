...
Occurrence records that carry DNA or RNA sequences have sequences sanitized before indexing, producing a normalised sequence together with a set of quality metrics. See the xref:sequence-sanitization.adoc[DNA sequence sanitization] page for the cleaning pipeline and the metrics it reports.
endif::[]
