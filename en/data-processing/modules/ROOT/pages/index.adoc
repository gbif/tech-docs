...
Occurrence records that carry DNA or RNA sequences have sequences sanitised before indexing, producing a normalised sequence together with a set of quality metrics. See the xref:sequence-sanitisation.adoc[DNA sequence sanitisation] page for the cleaning pipeline and the metrics it reports.
endif::[]
