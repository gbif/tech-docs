...
Each one of these steps can be in three possible states:
...
- Occurrence - main interpretation: Equivalent to re run VERBATIM TO INTERPRETATION
- Occurrence - indexing:  Equivalent to re run INTERPRETED TO INDEX
- Occurrence - creating table files: Equivalent to run HDFS VIEW
- Saving raw data fragments: Equivalent to run FRAGMENTER
- Events - main interpretation: 
- Events - indexing: 
- Events - creating table files: 

There are two additional options to modify the running of steps, those are:

Use the latest successful attempt: Run the steps using the latest attempt instead of a new one
Exclude event steps: Run the steps without using events
