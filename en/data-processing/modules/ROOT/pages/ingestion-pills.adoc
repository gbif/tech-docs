...
image::ingestion_steps.png[ingestion_steps,640,480, title="Not all steps will be present for every dataset, only the ones relevant for a particular origin and type."]
...
image::ingestion_states.png[ingestion_steps,640,480, title="Each state is represented by a different color, SUBMITTED: grey, RUNNING: green, QUEUED: yellow, COMPLETED: blue, FAILED: red, ABORTED: black."]

Finally, you can review the full results of the pipelines in the public ElasticSearch logs, where there is detailed information for each part of the process.

=== Re ingestion comments

Related to those steps, the registry can be used to run specific steps in pipeline, and the available steps are:
...
= MISSING PART
The suggested comment when you are using any of those steps is:

image::ingestion_run-steps.png[ingestion_steps,300,600, title="Ingestion re run steps on the registry"]
