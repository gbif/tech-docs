...
* https://www.gbif.org/dataset/d7dddbf4-2cf0-4f39-9b2a-bb099caae36c[GBIF backbone] - This is a taxonomy that up until recently GBIF has been building and integrating multiple times per year. It is primarily based on the http://www.catalogueoflife.org/[Catalogue of Life]. Higher-level classification above the family level exclusively comes from the Catalogue of Life, while the lower taxa was added in an automated way from other taxonomic datasets. The build process has now been discontinued in favour of the eXtended release of the Catalogue of Life. The GBIF Backbone will no longer be updated, but will remain available for backwards compatibility. All GBIF Backbone identifiers will be preserved and supported in the API.
...
Here is an example response for a match against COL XR:
[source,json]
----
{
  "usage": {
...
The REST Occurrence API response has been extended to include the taxonomic information from multiple taxonomies. The `classifications` array contains the taxonomic information from all taxonomies that were used to match the occurrence record. Note: the example response has been shortened for readability.
...
With every update of a taxonomy, versioned docker containers are created to support the species matching API. These containers are made available through our docker registry and can be used locally without depending on our online services.

The indexes in these docker containers are built using the data from the respective checklist in ChecklistBank.
ChecklistBank provides all the organised taxonomic information, names and hierarchy.
In addition, stable unique identifiers from other taxonomic datasets such as Dyntaxa, ITIS, WoRMs are retrieved from ChecklistBank and linked to taxa in the checklist.
...
The IUCN RedList category is also linked to taxa in the index. The IUCN RedList information is pulled from ChecklistBank when the docker containers are built.


=== Docker containers

You can list all available matching containers from the GBIF image registry https://docker.gbif.org/v2/matching-ws/tags/list[here]

The image tags are made up of several pieces of information:

`{taxonomy}-{architecture}-{datasetKey}-{date}-{time}`

So the image `xcol-arm64-308651-20250516-145444` exposes the COL eXtended Release with datasetKey=308651 and was build on the 16th of May 2025.
