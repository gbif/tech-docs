...
ifeval::["{env}" != "prod"]
[[DSI-decision]]
IMPORTANT: If occurrences in the dataset are based on DNA, it is required (where known) to share either <<country>>, <<countryCode>> or coordinates (<<decimalLatitude>>, <<decimalLongitude>>). 
...
^*^ As specified in the xref:DSI-decision[information box above], publishers may be required to share one or more of these terms. 
...
^*^ The publisher must provide either <<country>>, <<countryCode>> or coordinates (<<decimalLatitude>>, <<decimalLongitude>>), if known.
