...
[NOTE.material]
====
https://dwc.tdwg.org/terms/#materialentity[MaterialEntity^]
====
...
NOTE: Data published with the support of one of the programmes or projects managed by GBIF (e.g. https://www.gbif.org/programme/82243[BID], https://www.gbif.org/programme/82629[BIFA], https://www.gbif.org/programme/82219[CESP], https://www.gbif.org/news/59knqJp4BWbh6gnFh7E8qR/gbif-advances-edna-data-integration-to-support-global-research-and-decision-making[SPLICE]) have stricter data quality requirements than data published outside of these programmes.
...
ifeval::["{env}" != "prod"]
=== Data quality requirements for DNA reference libraries
Publishers who wish to share DNA reference libraries with GBIF must publish them as https://github.com/gbif/tech-docs/edit/main/en/data-publishing/modules/ROOT/pages/data-quality-recommendations.adoc#data-quality-requirements-for-occurrences[occurrences] with the https://rs.gbif.org/extension/gbif/1.0/dna_derived_data_2024-07-11.xml[DNA-derived data extension]. A DNA reference library is any curated or computationally assembled collection of taxonomically annotated DNA sequences, derived from physical specimens or other biological sources, used as a basis for identifying or classifying unknown biological material. 

.Data quality requirements for *DNA reference libraries*
[%autowidth,stripes=hover]
|===
|Term |<<Status>> |Status GBIF funded projects 
...
==== https://dwc.tdwg.org/terms/#dwc:higherClassification[_dwc:higherClassification_]
...
==== https://dwc.tdwg.org/list/#dwc_locality[_dwc:locality_]
...
=== {occurrence} catalogNumber [[catalogNumber]]
==== https://dwc.tdwg.org/list/#dwc_catalogNumber[_dwc:catalogNumber_]
...
=== {material} associatedSequences [[associatedSequences]]
==== https://dwc.tdwg.org/list/#dwc_associatedSequences[_dwc:associatedSequences_]

A list (concatenated and separated) of identifiers (publication, accession number, global unique identifier, URI) of genetic sequence information associated with the dwc:MaterialEntity.

'''

=== {identification} typeStatus [[typeStatus]]
==== https://dwc.tdwg.org/list/#dwc_typeStatus[_dwc:typeStatus_]

The nomenclatural type status of a specimen or name. Please choose a value from the GBIF controlled vocabulary for https://registry.gbif.org/vocabulary/TypeStatus[type status].

'''

=== {identification} identifiedBy [[identifiedBy]]
==== https://dwc.tdwg.org/list/#dwc_identifiedBy[_dwc:identifiedBy_]
...
==== https://rs.gbif.org/extension/gbif/1.0/dna_derived_data_2024-07-11.xml#DNA_sequence[_DNA_sequence_]

A nucleotide sequence composed exclusively of uppercase IUPAC nucleotide characters (G, A, T, C, R, Y, M, K, S, W, H, B, V, D, N), provided in 5′–3′ orientation, with no gap characters, whitespace, quality scores, sequence descriptions, or identifiers. Intended for use in taxon identification and biodiversity assessment. The target gene or barcode locus from which the sequence is derived should be specified using the <<target_gene>> term. 

'''

=== target_gene [[target_gene]]
==== 	https://w3id.org/mixs/0000044[_mixs:target_gene_]

Targeted gene or locus name for marker gene studies. Please use a value from https://registry.gbif.org/vocabulary/target_gene[GBIF's controlled vocabulary for target_gene].
...
The terms constitute the minimum formal requirements for publishing an occurrence dataset. GBIF will not accept a dataset without these terms and will not index the records. While these items are mandatory for publishing the dataset, they are only the starting point. The usefulness of the published data will remain severely limited unless additional information is provided.

=== Conditionally required 

The term(s) may be required if certain conditions are met. For example:

- At least one of either <<country>>, <<countryCode>> or coordinates (<<decimalLatitude>>, <<decimalLongitude>>) must be shared, if known, if the record is based on DNA evidence, to comply with the Convention on Biological Diversity https://www.cbd.int/doc/decisions/cop-16/cop-16-dec-02-en.pdf[decision] on digital sequence information (DSI) on genetic resources. 
...
* more precise geo-location data (coordinates) significantly increases the usefulness of the data for a wide range of use cases
