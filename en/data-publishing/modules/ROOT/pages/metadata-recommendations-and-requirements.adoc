...
== Test of extracting thing directly

This metadata list is pulled dynamically from the live schema definition profile and categorized automatically:

++++
<div id="xsd-loading-status" style="margin: 20px 0; font-weight: bold; color: #666;">
  🔄 Loading and grouping metadata terms directly from the official GBIF schema...
</div>

<div id="xsd-grouped-tables-container"></div>

<script>
document.addEventListener("DOMContentLoaded", function() {
  const xsdUrl = "https://rs.gbif.org/schema/eml-gbif-profile/1.3/eml-gbif-profile.xsd";
  
  // Define your precise subsection buckets
  const groups = {
    "Dataset (Resource)": [],
    "Project": [],
    "People and Organizations": [],
  ...
  // Helper function to figure out where an element structurally belongs
  function determineCategory(elem, name) {
    let current = elem.parentElement;
    
    // 1. Walk up the XML tree to check parent layout context
    while (current) {
      const parentName = current.getAttribute("name") || "";
      const parentType = current.getAttribute("type") || "";
      const combinedContext = (parentName + " " + parentType).toLowerCase();
      
      if (combinedContext.includes("project")) return "Project";
      if (["creator", "contact", "metadataprovider", "associatedparty", "agent", "personnel"].some(k => combinedContext.includes(k))) {
        return "People and Organizations";
      }
      if (combinedContext.includes("taxonomic")) return "Taxonomic Coverage";
      if (combinedContext.includes("geographic")) return "Geographic Coverage";
      if (combinedContext.includes("temporal")) return "Temporal Coverage";
      if (combinedContext.includes("methods") || combinedContext.includes("methodstep")) return "Methods";
      if (combinedContext.includes("keyword")) return "KeywordSet (General Keywords)";
      if (combinedContext.includes("coverage") && !combinedContext.includes("geographic") && !combinedContext.includes("temporal") && !combinedContext.includes("taxonomic")) {
        return "Coverage";
      }
    ...
    // 2. Explicit direct name mapping fallback
    const nameLower = name.toLowerCase();
    if (["alternateidentifier", "title", "pubdate", "language", "abstract", "purpose", "maintenanceupdatefrequency", "shortname"].includes(nameLower)) return "Dataset (Resource)";
    if (["intellectualrights"].includes(nameLower)) return "Intellectual Property Rights";
    if (["keywordset", "keyword", "keywordthesaurus"].includes(nameLower)) return "KeywordSet (General Keywords)";
    if (["coverage"].includes(nameLower)) return "Coverage";
    
    // Default bucket for physical elements and specialized collection identifiers
    return "Additional Metadata + Natural Collections Description Data (NCD) Related";
  }

  fetch(xsdUrl)
    .then(response => {
      if (!response.ok) throw new Error("Network configuration block or response failure");
    ...
    .then(xmlString => {
      ...
      for (let i = 0; i < elements.length; i++) {
        ...
        const docNode = elem.getElementsByTagNameNS("*", "documentation")[0];
        if (docNode && docNode.textContent) {
          const description = docNode.textContent.trim().replace(/\s+/g, ' ');
          const targetGroup = determineCategory(elem, name);
          
          // Deduplicate definitions within the same category block
          const uniqueKey = `${targetGroup}:${name}`;
          if (seenEntries.has(uniqueKey)) continue;
          seenEntries.add(uniqueKey);
          
          // Generate native Antora-styled table rows
          const rowHtml = `<tr>
            <td class="tableblock halign-left valign-top" style="width: 30%;"><p class="tableblock"><code>${name}</code></p></td>
      ...
      // Hide the loading warning
      document.getElementById("xsd-loading-status").style.display = "none";
      
      // Build grouped HTML outputs
      let finalHtml = "";
      for (const [groupName, rows] of Object.entries(groups)) {
        if (rows.length === 0) continue; // Skip displaying empty groups cleanly
        
        finalHtml += `<h3 class="discrete" style="margin-top: 2.5rem; color: #2d3748; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem;">${groupName}</h3>`;
        finalHtml += '<table class="tableblock frame-all grid-all stretch" style="margin-top: 1rem; margin-bottom: 1.5rem;">';
        finalHtml += '<thead><tr><th class="tableblock halign-left valign-top">Field Name</th><th class="tableblock halign-left valign-top">Description</th></tr></thead>';
        finalHtml += '<tbody>';
        finalHtml += rows.join('');
        finalHtml += '</tbody></table>';
      }
      
      document.getElementById("xsd-grouped-tables-container").innerHTML = finalHtml;
    })
    .catch(err => {
      console.error("XSD Processing Error:", err);
      document.getElementById("xsd-loading-status").innerHTML = `
        <div style="padding: 15px; background: #fff3cd; color: #856404; border: 1px solid #ffeeba; border-radius: 4px;">
          ⚠️ <strong>Note:</strong> Grouped terms couldn't render directly. 
          Review the structural schema file directly at the <a href="${xsdUrl}" target="_blank" style="text-decoration: underline; color: #533f03;">GBIF Profile Registry</a>.
        </div>`;
    });
});
</script>
