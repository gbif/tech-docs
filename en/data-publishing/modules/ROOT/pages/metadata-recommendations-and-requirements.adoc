...
This metadata list is pulled dynamically from the live schema definition profile:

++++
<div id="xsd-table-container" style="margin: 20px 0; font-weight: bold; color: #666;">
  🔄 Loading metadata terms directly from the official GBIF schema...
</div>
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  fetch(xsdUrl)
    ...
    .then(xmlString => {
      ...
      // Grabs all element nodes regardless of whether they use 'xs:' or 'xsd:' prefixes
      const elements = xmlDoc.getElementsByTagNameNS("*", "element");
      
      // Build an Antora-styled HTML table so it matches your theme perfectly
      let html = '<table class="tableblock frame-all grid-all stretch">';
      html += '<thead><tr><th class="tableblock halign-left valign-top">Field Name</th><th class="tableblock halign-left valign-top">Description</th></tr></thead>';
      html += '<tbody>';
      ...
      for (let i = 0; i < elements.length; i++) {
        ...
        if (docNode && docNode.textContent) {
          // Clean up spaces, newlines, and tabs
          const description = docNode.textContent.trim().replace(/\s+/g, ' ');
          
          html += `<tr>`;
          html += `<td class="tableblock halign-left valign-top"><p class="tableblock"><code>${name}</code></p></td>`;
          html += `<td class="tableblock halign-left valign-top"><p class="tableblock">${description}</p></td>`;
          html += `</tr>`;
          elementCount++;
        }
      }
      ...
      if (elementCount > 0) {
        document.getElementById("xsd-table-container").innerHTML = html;
      } else {
        throw new Error("No documented elements found");
      }
    })
    .catch(err => {
      console.error("XSD Load Error:", err);
      // Clean fallback so the user still gets a helpful link if something goes wrong
      document.getElementById("xsd-table-container").innerHTML = `
        <div style="padding: 15px; background: #fff3cd; color: #856404; border: 1px solid #ffeeba; border-radius: 4px;">
          ⚠️ <strong>Note:</strong> Unable to load the terms inline automatically. 
          You can view the official live schema rules directly at the 
          <a href="${xsdUrl}" target="_blank" style="text-decoration: underline; color: #533f03;">GBIF Profile Registry</a>.
        </div>`;
    });
});
</script>
