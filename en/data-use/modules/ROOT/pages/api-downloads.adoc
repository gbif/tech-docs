...
ifeval::["{env}" != "prod"]
`format` can be changed to one of `SIMPLE_CSV`, `DWCA`, `SPECIES_LIST` or `FASTA_ARCHIVE`, see xref::download-formats.adoc[Occurrence download formats].
endif::[]
ifeval::["{env}" == "prod"]
`format` can be changed to one of `SIMPLE_CSV`, `DWCA` or `SPECIES_LIST`, see xref::download-formats.adoc[Occurrence download formats].
endif::[]
...
----
{
  ...
  "predicate": {
    ...
    "predicates": [
      ...
      { "type": "equals", "key": "TAXON_KEY", "value": "59R5F", "checklistKey": "7ddf754f-d193-4cc9-b351-99906754a03b" }
    ]
  }
}
----
...
=== Predicate support for multiple taxonomies

the `equals`, `in` support multiple taxonomies by adding a `checklistKey` property to the predicate.
...
Users can specify the taxonomy to be included in occurrence downloads by adding the `checklistKey` parameter to the download request. *If no `checklistKey` is supplied the obsolete GBIF Backbone Taxonomy will be used.* The `checklistKey` should be specified for the whole request (for the taxonomy used for output) as well as in taxonomic filters.
...
----
{
  ...
  "sendNotification": true,
  "format": "SIMPLE_CSV",
  "checklistKey": "7ddf754f-d193-4cc9-b351-99906754a03b" <1>
  "predicate": {
    "type": "equals",
    "key": "TAXON_KEY",
    "checklistKey": "7ddf754f-d193-4cc9-b351-99906754a03b", <2>
    "value": "5WZLF"
  }
}
----
<1> Use the https://www.gbif.org/dataset/7ddf754f-d193-4cc9-b351-99906754a03b[Catalogue of Life] for the output, i.e. `species`, `scientificName`, `taxonKey` etc columns
<2> Use the Catalogue of Life for filtering to taxon https://www.gbif.org/taxon/5WZLF[5WZLF (_Carcharodon carcharias_)].
...
Some extensions are also available in the form processed by GBIF.  These are requested with an `interpretedExtensions` property, and are included in the archive as ordinary extension tables, described in `meta.xml`:
...
The DNA Derived Data extension is added to the archive as `dnaderiveddata.txt`, with the columns `gbifID`, `target_gene` and `dna_sequence`.  Unlike the verbatim table, the target gene is normalised against the GBIF https://registry.gbif.org/vocabulary/target_gene[target gene vocabulary] and the sequence is the sanitized one — see xref:data-processing::sequence-sanitization.adoc[DNA sequence sanitization].  The other terms of the extension are not repeated here; request the verbatim extension as well to get them as published.
endif::[]
...
A download in `FASTA_ARCHIVE` format is a Darwin Core Archive with the sanitized sequences added in FASTA format, together with their quality metrics.  It is described in detail on xref::download-formats.adoc#fasta[Occurrence download formats].

The format restricts the download to occurrences with at least one sanitized sequence, so no predicate is needed for that.  Filters on the sequences themselves use the `NUCLEOTIDE_SEQUENCE_*` keys, for example to request the COI sequences of Insecta in Norway:
...
----
{
  ...
  "predicate": {
    ...
    "predicates": [
      {
        "type": "equals",
        "key": "NUCLEOTIDE_SEQUENCE_TARGET_GENE",
        "value": "COI"
      },
      {
        "type": "equals",
  ...
  "verbatimExtensions": [
    "http://rs.gbif.org/terms/1.0/DNADerivedData" <1>
  ]
}
----
<1> Optional.  Adds the DNA Derived Data extension as published, including the MIxS terms and the original sequences.

The `target_gene`, taxonomic and geographic filters of the request are also used to build the citation of the download, so a request phrased in these terms produces a citation describing what the archive contains.
endif::[]
