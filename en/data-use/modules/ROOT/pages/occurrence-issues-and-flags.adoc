...
These flags relate to the sanitisation of DNA and RNA sequences published in the https://rs.gbif.org/extension/gbif/1.0/dna_derived_data_2024-07-11.xml[DNA derived data extension]. Unlike the issues above, they are not part of the occurrence issue vocabulary: they are reported as fields on each `nucleotideSequence` object and can be used as search filters (for example `nucleotideSequence.invalid=true`). See xref:data-processing::sequence-sanitisation.adoc[DNA sequence sanitisation] for the full cleaning pipeline and the metrics that accompany these flags.
...
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?nucleotideSequence.invalid=true[example]

|*Natural language detected*
|Marker words associated with laboratory metadata rather than nucleotides (for example `FWD`, `REVERSE`, `MERGED`, `UNMERGED`) were found in the sequence. The record is treated as invalid.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?nucleotideSequence.naturalLanguageDetected=true[example]

|*Sequence ends trimmed*
...
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?nucleotideSequence.endsTrimmed=true[example]

|*Gaps or whitespace removed*
|Whitespace and/or alignment gap characters (`-`, `.`) were removed from the sequence during sanitisation.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?nucleotideSequence.gapsOrWhitespaceRemoved=true[example]

|*N-runs capped*
|One or more long runs of `N` (ambiguous bases) were shortened to a fixed maximum length during sanitisation. The number of runs shortened is reported in `nRunsCapped`.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?nucleotideSequence.nRunsCapped=1[example]
|===
endif::[]
