...
These issues are raised during the sanitisation of DNA and RNA sequences published in the https://rs.gbif.org/extension/gbif/1.0/dna_derived_data_2024-07-11.xml[DNA derived data extension]. Like the other issues on this page, they can be used as occurrence search filters (for example `issue=NUCLEOTIDE_SEQUENCE_INVALID`). See xref:data-processing::sequence-sanitisation.adoc[DNA sequence sanitisation] for the full cleaning pipeline and the metrics that accompany these issues.
...
|The sequence could not be sanitised into a usable form: after cleaning it still contains characters that are not valid IUPAC DNA codes (`nonIupacFraction` > 0), or natural language was detected. Invalid sequences are not indexed — both `sequence` and `nucleotideSequenceID` are set to `null` — while the quality metrics are still reported.
|_DNA_sequence_
...
|Leading and/or trailing characters were removed during sanitisation, trimming the sequence back to a run of recognisable nucleotide anchor characters at each end.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?issue=NUCLEOTIDE_SEQUENCE_ENDS_TRIMMED[example]

|*Nucleotide sequence gaps removed*
|Gap characters (`-`, `.`) and/or whitespace were removed from the published sequence during sanitisation.
|_DNA_sequence_
...
|More than 1% of the characters in the cleaned sequence are ambiguous IUPAC codes (anything other than A, C, G, T or N): `nonACGTNFraction` > 0.01.
|_DNA_sequence_
