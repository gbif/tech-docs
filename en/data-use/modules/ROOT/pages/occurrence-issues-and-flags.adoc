...
// The definitive list of flags in the API, and can be seen in the source code:
...
a|The recording date given cannot be interpreted because it is invalid.
...
a|The identification date given cannot be interpreted because it is invalid.
...
ifeval::["{env}" != "prod"]
|*Target gene invalid*
|The supplied target gene could not be matched against the GBIF https://registry.gbif.org/vocabulary/target_gene[target gene vocabulary] (https://api.gbif.org/v1/vocabularies/target_gene[API]).
|_target_gene_
|https://www.gbif.org/occurrence/search?issue=TARGET_GENE_INVALID[example]
endif::[]
...
These issues are raised during the sanitization of DNA and RNA sequences published in dataset records. Like the other issues on this page, they can be used as occurrence search filters (for example `issue=NUCLEOTIDE_SEQUENCE_INVALID`). See xref:data-processing::sequence-sanitization.adoc[DNA sequence sanitization] for the full cleaning pipeline and the metrics that accompany these issues.

[cols="20%,60%,10%,10%"]
|===
|Flag name | Definition | Terms | Example

|*Nucleotide sequence invalid*
|The sequence could not be sanitized into a usable form: after cleaning, it still contains characters that are not valid IUPAC DNA codes (`nonIupacFraction` > 0), or natural language was detected. Invalid sequences are not indexed — both `sequence` and `nucleotideSequenceID` are set to `null` — while the quality metrics are still reported.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?issue=NUCLEOTIDE_SEQUENCE_INVALID[example]

|*Nucleotide sequence natural language*
|Marker words associated with laboratory metadata rather than nucleotides (for example `FWD`, `REVERSE`, `MERGED`, `UNMERGED`) were found in the published sequence. The sequence is treated as invalid.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?issue=NUCLEOTIDE_SEQUENCE_NATURAL_LANGUAGE[example]

|*Nucleotide sequence ends trimmed*
|Leading and/or trailing characters were removed during sanitization, trimming the sequence back to a run of recognisable nucleotide anchor characters at each end.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?issue=NUCLEOTIDE_SEQUENCE_ENDS_TRIMMED[example]

|*Nucleotide sequence gaps removed*
|Gap characters (`-`, `.`) and/or whitespace were removed from the published sequence during sanitization.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?issue=NUCLEOTIDE_SEQUENCE_GAPS_REMOVED[example]

|*Nucleotide sequence high N fraction*
|More than 5% of the characters in the cleaned sequence are `N` (unknown bases): `nFraction` > 0.05.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?issue=NUCLEOTIDE_SEQUENCE_HIGH_N_FRACTION[example]

|*Nucleotide sequence high ambiguity*
|More than 1% of the characters in the sanitized sequence are ambiguous IUPAC codes (anything other than A, C, G, T or N): `nonACGTNFraction` > 0.01.
|_DNA_sequence_
|https://www.gbif.org/occurrence/search?issue=NUCLEOTIDE_SEQUENCE_HIGH_AMBIGUITY[example]
