...
Note this grid is identical to the <<eurostat-grid-cell-code,EuroStat Grid>>, but with different identifiers.
...
=== EuroStat Grid cell code

*Suggested function declaration:*

[source,sql]
----
CREATE OR REPLACE TEMPORARY FUNCTION euroStatCellCode AS 'org.gbif.occurrence.hive.udf.EuroStatCellCodeUDF';
----
...
[source,sql]
----
STRING euroStatCellCode(INTEGER gridSize,
                        DOUBLE latitude,
                        DOUBLE longitude,
                        DOUBLE coordinateUncertaintyInMeters)
----
...
`gridSize`:: The _grid size_ in metres of the grid.  Must be _1_, _2_, _5_, _10_, _20_, _50_, _100_, _200_, _500_, _1000_, _2000_, _5000_, _10000_, _50000_ or _100000_.
`latitude`:: The latitude of the _point_ to grid (usually the `decimalLatitude` field)
`longitude`:: The longitude of the _point_ to grid (usually the `decimalLongitude` field)
`coordinateUncertaintyInMeters`:: The uncertainty radius to apply to the _point_ (usually used with a default value, for example `COALESCE(coordinateUncertaintyInMeters, 1000)`)
...
The code for the cell of the EuroStat ETRS89-LAEA grid cell in which the randomized point falls.
...
Note this grid is identical to the <<eea-reference-grid-cell-code,EEA Reference Grid>>, but with different identifiers.

GeoPackage and ShapeFile downloads of the grids at several resolutions are available for download at the https://ec.europa.eu/eurostat/web/gisco/geodata/grids[EuroStat website].

Reference: https://knowledge-base.inspire.ec.europa.eu/publications/inspire-data-specification-statistical-units-technical-guidelines_en[INSPIRE Data Specification on Statistical Units – Technical Guidelines].
