...
ifeval::["{env}" != "prod"]
* xref:v2@openapi::taxon.adoc[*Taxon*] provides #TODO# services to discover and access information about species and higher taxa, and utility services for interpreting names and looking up the identifiers and complete scientific names used for species in the GBIF portal. #TODO#
endif::[]
...
ifeval::["{env}" != "prod"]
* xref:v1@openapi::event.adoc[*Event*] provides #TODO# #TODO#
endif::[]
* The xref:openapi::images.adoc[*Occurrence Image API*] provides low or medium resolution images linked to occurrences.
...
*Rapid or numerous queries to search APIs may be rate limited*, depending on our server load.  If this happens, you will receive an `HTTP 429` error, and you should reduce your query rate or use the xref:data-use::api-downloads.adoc[download API].  The maximum rate changes depending on our server load; we do not guarantee any particular query rate will work.

The most common cause of rate-limiting is when retrieving occurrence records for thousands or tens of thousands of species/countries/datasets/locations.  *If a script will take more than 15 minutes using the occurrence search APIs it will be usually be faster and cause less load on our servers to xref:data-use::api-downloads.adoc[request a download] instead.*  This also allows for proper https://www.gbif.org/citation-guidelines[attribution and citation].  The download API accepts up to 100,000 search parameters, such as taxa or datasets.

== Unsupported use

API endpoints not listed in the documentation, or those marked "experimental", may be changed or removed without warning.
