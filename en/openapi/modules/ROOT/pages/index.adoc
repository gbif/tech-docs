...
Frequent queries to search APIs may be rate limited, depending on our server load.  If this happens, you will receive an HTTP `429` error, and you should reduce your query rate and try again.

The most common cause of rate-limiting is users trying to retrieve occurrence records for thousands or tens of thousands of species/countries/datasets/locations.  A single xref:data-use::api-downloads.adoc[occurrence download query] accepts 100,000 parameters and should be used instead.
