...
This directory contains a copy of the https://github.com/LiskHQ/lisk-docs/tree/antora[Antora branch] in Lisk with a few quick hacks applied (banner color and GBIF logo) for a quick demo.
It is to be refactored or replaced correctly.

Please note that the menu structure is found in this theme, and that the theme carries an MPL2 license.

To build this without installing Node/NVM etc you can run the following the theme directory:

----
docker run -it --rm -v $(pwd):/srv huli/gulp bundle
----

The resulting build/ui-bundle.zip is referenced in the Antora playbook in the project root.
